...
const PaymentDueCard: React.FC = () => {
  ...
  // Don't show if fully paid, loading, or no config
  if (profile?.payment_status !== 'CONFIRMED_15K' || isLoading || !paymentConfig) {
    return null;
  }

  const { programme_total, deposit_paid, deposit_label, balance_due, payment_deadline, payment_link, installment_link } = paymentConfig;
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card/80 backdrop-blur-sm overflow-hidden">
      ...
      <div className="px-5 pt-4 pb-0">
        <span className="inline-block px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full bg-destructive/15 text-destructive border border-destructive/20">
          Payment Due
        </span>
      </div>
