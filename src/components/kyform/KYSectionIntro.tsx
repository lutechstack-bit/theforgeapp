...
export const KYSectionIntro: React.FC<KYSectionIntroProps> = ({ section }) => {
  return (
    <div className="flex flex-col items-center text-center px-2 py-4 space-y-4">
      ...
      <div className="w-16 h-16 rounded-2xl bg-forge-gold/10 border-2 border-forge-gold/25 flex items-center justify-center text-3xl shadow-[0_0_30px_-8px_hsl(var(--forge-gold)/0.3)]">
        {section.icon}
      </div>
        ...
        <div className="w-full max-w-sm space-y-2">
          ...
          <div className="space-y-1.5">
            {section.keepHandy.map((item, index) => (
              <div
                key={index}
                className="flex items-center gap-2.5 px-3 py-2 rounded-xl bg-forge-gold/5 border border-forge-gold/15"
              >
